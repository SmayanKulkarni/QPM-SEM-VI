...
with pdfplumber.open("/mnt/user-data/uploads/scan.pdf") as pdf:
    page = pdf.pages[0]
...
Rasterize each page at 2x zoom, then use the `view` tool to read them visually.
Claude's vision can read printed text, formulas, tables, and diagrams all at once.

```python
import fitz  # PyMuPDF

doc = fitz.open("/mnt/user-data/uploads/scan.pdf")
