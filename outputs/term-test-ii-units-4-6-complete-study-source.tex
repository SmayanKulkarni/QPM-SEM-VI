...
\noindent\textbf{Example 1}\enspace\textit{equal allocation}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Total investment = INR 1,00,000
  \item Assets = 5
  \item Weight per asset = 1/5 = 20\%
  \item Allocation per asset = INR 20,000
\end{itemize}

\noindent\textbf{Interpretation:} pure equal-weight capital split.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{portfolio return from equal weights}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item 4 stocks, equal weight 25\% each
  \item Returns = 8\%, 12\%, 10\%, 14\%
  \item Portfolio return = 0.25$\times$8 + 0.25$\times$12 + 0.25$\times$10 + 0.25$\times$14 = 11\%
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{return in rupee terms}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Total capital = INR 3,00,000
  \item 5 assets, equal allocation $\Rightarrow$ INR 60,000 each
  \item Mean portfolio return shown in source = 11\%
  \item Total rupee return = 3,00,000 x 0.11 = INR 33,000
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{empirical comparison linkage}

\begin{center}
\includegraphics[width=0.8\linewidth]{figures/fig_0123.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0124.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0125.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0126.png}
\end{center}


\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Equal weight used as diversification benchmark in optimization-vs-diversification deck.
  \item Source case reports equal-weight expected return near 11.34\% versus concentrated optimized portfolio near 12.20\%.
\end{itemize}

\noindent\textbf{Interpretation:} higher expected return may come with concentration risk.
...
Three measures:
\begin{enumerate}
\item \textbf{Marginal Contribution to Risk} ($\text{MCR}_i$):
\[
\text{MCR}_i = \frac{\text{Cov}(R_i,\,R_p)}{\sigma_p}
\]
Meaning: how much total portfolio risk changes for a marginal weight increase in asset $i$.

\item \textbf{Total Risk Contribution} ($\text{TRC}_i$):
\[
\text{TRC}_i = w_i \times \text{MCR}_i
\]
Meaning: absolute risk contribution of asset $i$.

\item \textbf{Percentage Risk Contribution} ($\text{PRC}_i$):
\[
\text{PRC}_i = \frac{\text{TRC}_i}{\sigma_p}
\]
Meaning: share of total portfolio risk attributable to asset $i$.
\end{enumerate}
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{two-asset case}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $w_A=0.60$,\; $\text{Cov}(A,p)=0.018$,\; $\sigma_p=0.12$
  \item $w_B=0.40$,\; $\text{Cov}(B,p)=0.012$,\; $\sigma_p=0.12$
  \item $\text{MCR}_A = 0.018/0.12 = 0.15$
  \item $\text{MCR}_B = 0.012/0.12 = 0.10$
  \item $\text{TRC}_A = 0.60 \times 0.15 = 0.09$
  \item $\text{TRC}_B = 0.40 \times 0.10 = 0.04$
  \item $\text{PRC}_i = \text{TRC}_i / \sigma_p$ (computed from above values).
\end{itemize}

\noindent\textbf{Interpretation:} Asset A dominates risk share.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{alternate two-asset setup}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\sigma_p=0.10$,\; $\text{Cov}(A,p)=0.018$,\; $\text{Cov}(B,p)=0.012$; same weights as Example~1.
  \item $\text{MCR}_A=0.18$,\; $\text{MCR}_B=0.12$
  \item $\text{TRC}_A=0.108$,\; $\text{TRC}_B=0.048$
  \item $\text{PRC}_A = \text{TRC}_A/\sigma_p$ is disproportionately large $\Rightarrow$ strong concentration in A.
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{three-asset fund: MCR, TRC, PRC full calculation}

\noindent\textbf{Given:} $w_E=0.50$, $w_B=0.30$, $w_C=0.20$; $\sigma_p=0.14$;
$\text{Cov}(E,p)=0.022$, $\text{Cov}(B,p)=0.006$, $\text{Cov}(C,p)=0.008$.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\text{MCR}_E = 0.022/0.14 = 0.157$;\quad $\text{MCR}_B = 0.006/0.14 = 0.043$;\quad $\text{MCR}_C = 0.008/0.14 = 0.057$
  \item $\text{TRC}_E = 0.50\times0.157 = 0.079$;\quad $\text{TRC}_B = 0.30\times0.043 = 0.013$;\quad $\text{TRC}_C = 0.20\times0.057 = 0.011$
  \item $\text{PRC}_E = 0.079/0.14 = 56.4\%$;\quad $\text{PRC}_B = 9.3\%$;\quad $\text{PRC}_C = 7.9\%$
\end{itemize}

\noindent\textbf{Interpretation:} Equity consumes 56.4\% of total portfolio risk despite holding only 50\% of capital. Rebalancing needed to achieve a balanced risk profile.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{risk-control decision using PRC}

\noindent\textbf{Given:} From Example 3: $\text{PRC}_E=56.4\%$, $\text{PRC}_B=9.3\%$, $\text{PRC}_C=7.9\%$.\\
Target risk budget: Equity 40\%, Bond 35\%, Commodity 25\%.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Equity over-budget: actual 56.4\% vs target 40\% $\Rightarrow$ reduce $w_E$ from 0.50 toward 0.38.
  \item Bond under-budget: actual 9.3\% vs target 35\% $\Rightarrow$ increase $w_B$ from 0.30 toward 0.45.
  \item Commodity under-budget: actual 7.9\% vs target 25\% $\Rightarrow$ increase $w_C$ from 0.20 toward 0.30.
\end{itemize}

\noindent\textbf{Interpretation:} Risk budgeting forces rebalancing based on risk consumed, not capital deployed. The portfolio is reallocated away from Equity until its PRC matches its target risk share.
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{equity-bond-commodity}

\begin{center}
\includegraphics[width=0.8\linewidth]{figures/fig_0025.png}
\end{center}


\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Risk contribution = weight x volatility
  \item Equity: 0.50$\times$0.20=0.10
  \item Bond: 0.30$\times$0.10=0.03
  \item Commodity: 0.20$\times$0.15=0.03
  \item Total risk=0.16
  \item Risk shares: 62.5\%, 18.75\%, 18.75\%
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{capital-to-risk decomposition (3 stocks)}

\noindent\textbf{Given:} INR~1,00,000 portfolio: Stock A = INR~50,000 ($w_A=0.50$); B = INR~30,000 ($w_B=0.30$); C = INR~20,000 ($w_C=0.20$). Volatilities: $\sigma_A=25\%$, $\sigma_B=15\%$, $\sigma_C=20\%$.

\noindent\textbf{Steps (simplified RC = $w_i\times\sigma_i$):}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\text{RC}_A = 0.50\times0.25 = 0.125$;\quad $\text{RC}_B = 0.30\times0.15 = 0.045$;\quad $\text{RC}_C = 0.20\times0.20 = 0.040$
  \item Total $\text{RC} = 0.125+0.045+0.040 = 0.210$
  \item Risk share A $= 0.125/0.210 = 59.5\%$;\quad B $= 21.4\%$;\quad C $= 19.0\%$
  \item Rupee risk: A $= 59.5\%\times1{,}00{,}000 =$ INR~59,500; B = INR~21,400; C = INR~19,000
\end{itemize}

\noindent\textbf{Interpretation:} Stock A uses 59.5\% of total risk budget despite having only 50\% capital allocation --- it is the dominant risk driver.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{risk budget from total risk}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Total risk 18\%; target shares 50/30/20
  \item Risk allocated: 9\%, 5.4\%, 3.6\%
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{target vs actual risk budget comparison}

\noindent\textbf{Given:} From Example 1 results: Equity risk share = 62.5\%, Bond = 18.75\%, Commodity = 18.75\%.\\
Target risk budget: Equity 40\%, Bond 35\%, Commodity 25\%.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Equity breach: $62.5\% - 40\% = +22.5\%$ over budget $\Rightarrow$ sell equity.
  \item Bond breach: $18.75\% - 35\% = -16.25\%$ under budget $\Rightarrow$ buy bonds.
  \item Commodity breach: $18.75\% - 25\% = -6.25\%$ under budget $\Rightarrow$ buy commodity.
\end{itemize}

\noindent\textbf{Interpretation:} The portfolio is heavily equity-dominated in risk terms. Rebalancing toward bonds and commodities reduces concentration and aligns with the mandated risk budget.
...
\textbf{Formulas:}
\[
q_i = \frac{1}{\sigma_i}, \qquad w_i = \frac{q_i}{\sum_j q_j}
\]
Lower $\sigma_i$ $\Rightarrow$ larger $q_i$ $\Rightarrow$ larger weight.
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{3-asset simplified risk parity}

\noindent\textbf{Given:} $\sigma_E=20\%$, $\sigma_B=10\%$, $\sigma_C=15\%$.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $q_E = 1/0.20 = 5.00$;\quad $q_B = 1/0.10 = 10.00$;\quad $q_C = 1/0.15 = 6.67$
  \item Sum $= 5.00+10.00+6.67 = 21.67$
  \item $w_E = 5.00/21.67 = 23.1\%$;\quad $w_B = 10.00/21.67 = 46.2\%$;\quad $w_C = 6.67/21.67 = 30.8\%$
\end{itemize}

\noindent\textbf{Interpretation:} Bonds receive the highest weight (46.2\%) because they have the lowest volatility (10\%). Equity receives the least capital (23.1\%) to prevent it from dominating portfolio risk.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{4-asset simplified risk parity}

\begin{center}
\includegraphics[width=0.8\linewidth]{figures/fig_0028.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0029.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0030.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0031.png}
\end{center}


\noindent\textbf{Given:} $\sigma_E=24\%$, $\sigma_B=8\%$, $\sigma_\text{Com}=16\%$, $\sigma_\text{RE}=12\%$.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $q_E=1/0.24=4.17$;\; $q_B=1/0.08=12.50$;\; $q_\text{Com}=1/0.16=6.25$;\; $q_\text{RE}=1/0.12=8.33$
  \item Sum $= 4.17+12.50+6.25+8.33 = 31.25$
  \item $w_E=4.17/31.25=13.3\%$;\; $w_B=12.50/31.25=40.0\%$;\; $w_\text{Com}=6.25/31.25=20.0\%$;\; $w_\text{RE}=8.33/31.25=26.7\%$
\end{itemize}

\noindent\textbf{Interpretation:} Bond receives 40\% of capital (lowest $\sigma$). Equity gets only 13.3\% despite potentially highest expected return, because its high volatility would otherwise dominate risk.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{3-asset investor case}

\begin{center}
\includegraphics[width=0.8\linewidth]{figures/fig_0032.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0033.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0034.png}
\end{center}


\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Stock index 20\%, Gov bonds 10\%, Gold 15\%
  \item Inverse vol $\Rightarrow$ 5,10,6.67; normalized weights around 23.07\%, 46.15\%, 30.78\%
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{4-asset RP allocation in rupees}

\begin{center}
\includegraphics[width=0.8\linewidth]{figures/fig_0035.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0036.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0037.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0038.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0039.png}
\end{center}


\noindent\textbf{Given:} Total capital = INR~10,00,000. RP weights from Example 2: $w_E=13.3\%$, $w_B=40.0\%$, $w_\text{Com}=20.0\%$, $w_\text{RE}=26.7\%$.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Equity: $10{,}00{,}000 \times 0.133 = $ INR~1{,}33{,}000
  \item Bond: $10{,}00{,}000 \times 0.400 = $ INR~4{,}00{,}000
  \item Commodity: $10{,}00{,}000 \times 0.200 = $ INR~2{,}00{,}000
  \item Real Estate: $10{,}00{,}000 \times 0.267 = $ INR~2{,}67{,}000
\end{itemize}

\noindent\textbf{Interpretation:} Compare to equal-weight (INR~2{,}50{,}000 each): RP shifts INR~1{,}17{,}000 away from equity into bonds, significantly reducing equity risk dominance.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 5}\enspace\textit{equal-weight vs risk-parity: side-by-side comparison}

\begin{center}
\includegraphics[width=0.8\linewidth]{figures/fig_0040.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0041.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0042.png}
\end{center}


\noindent\textbf{Given:} 4 assets from Example 2 ($\sigma_E=24\%$, $\sigma_B=8\%$, $\sigma_\text{Com}=16\%$, $\sigma_\text{RE}=12\%$).

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Equal-weight (EW): $w_i=25\%$ each. Risk contribution $\propto w_i\sigma_i$: E=6.0, B=2.0, Com=4.0, RE=3.0. Equity share = 6.0/15.0 = 40\%.
  \item Risk-parity (RP): $w_E=13.3\%$, $w_B=40\%$, $w_\text{Com}=20\%$, $w_\text{RE}=26.7\%$. RC: E=3.19, B=3.20, Com=3.20, RE=3.20 $\approx$ equal.
  \item EW equity risk share = 40\%; RP equity risk share $\approx$ 25\% (balanced).
\end{itemize}

\noindent\textbf{Interpretation:} Risk parity equalizes risk contributions across assets. EW gives equity disproportionate risk dominance (40\%). RP corrects this, providing genuinely diversified risk exposure.
...
\]
\[
\text{RC}_i = w_i\,(\boldsymbol{\Sigma}\mathbf{w})_i
\]
Target condition (full RP):
\[
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{3-asset portfolio variance decomposition}

\begin{center}
\textbf{Case 1: Full RP Optimization Case}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0043.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0044.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0045.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0046.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0047.png}\\
\vspace{0.5cm}
\textbf{Case 2: 3-Asset Variance and Contribution Case}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0048.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0049.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0050.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0051.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0052.png}
\end{center}


\noindent\textbf{Given:} $w_E=0.50$, $w_B=0.30$, $w_C=0.20$; $\sigma_E=20\%$, $\sigma_B=10\%$, $\sigma_C=15\%$; $\rho_{EB}=0.30$, $\rho_{EC}=0.20$, $\rho_{BC}=0.10$.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Covariances: $\text{Cov}_{EB}=0.30\times0.20\times0.10=0.0060$;\; $\text{Cov}_{EC}=0.0060$;\; $\text{Cov}_{BC}=0.0015$
  \item Variance terms: $w_E^2\sigma_E^2=0.25\times0.04=0.0100$;\; $w_B^2\sigma_B^2=0.09\times0.01=0.0009$;\; $w_C^2\sigma_C^2=0.04\times0.0225=0.0009$
  \item Cross terms: $2w_Ew_B\text{Cov}_{EB}=2(0.5)(0.3)(0.006)=0.0018$;\; $2w_Ew_C\text{Cov}_{EC}=0.0012$;\; $2w_Bw_C\text{Cov}_{BC}=0.00018$
  \item $\sigma_p^2 = 0.0100+0.0009+0.0009+0.0018+0.0012+0.00018 = 0.01498$
  \item $\sigma_p = \sqrt{0.01498} = 12.24\%$
\end{itemize}

\noindent\textbf{Interpretation:} Individual $\sigma$s are 20\%, 10\%, 15\% (weighted avg $\approx$16.5\%). Diversification reduces portfolio risk to 12.24\% --- a 4.3\% reduction from the weighted-average standalone risk.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{risk contribution by asset in full RP context}

\noindent\textbf{Given:} Same as Example 1. $\sigma_p=12.24\%$ computed above.

\noindent\textbf{Steps (compute $\text{RC}_i = w_i(\boldsymbol{\Sigma}\mathbf{w})_i$):}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $(\boldsymbol{\Sigma}\mathbf{w})_E = w_E\sigma_E^2 + w_B\text{Cov}_{EB} + w_C\text{Cov}_{EC} = 0.5(0.04)+0.3(0.006)+0.2(0.006) = 0.0218$
  \item $(\boldsymbol{\Sigma}\mathbf{w})_B = 0.5(0.006)+0.3(0.01)+0.2(0.0015) = 0.0063$
  \item $(\boldsymbol{\Sigma}\mathbf{w})_C = 0.5(0.006)+0.3(0.0015)+0.2(0.0225) = 0.0079$
  \item $\text{RC}_E = 0.5\times0.0218 = 0.0109$;\; $\text{RC}_B = 0.3\times0.0063 = 0.00189$;\; $\text{RC}_C = 0.2\times0.0079 = 0.00158$
  \item Risk shares: E = $0.0109/0.01498 = 72.8\%$; B = $12.6\%$; C = $10.5\%$ (sum $\approx\sigma_p^2$)
\end{itemize}

\noindent\textbf{Interpretation:} Despite equal-sounding weights, equity consumes 72.8\% of portfolio variance --- confirming that full RP must equalise $\text{RC}_i$, not $w_i$, to truly balance risk.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{risk-contribution interpretation}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Using same case outputs, explain why equal capital is not equal risk.
  \item Show why full RP decision variable must be risk contribution balance.
\end{itemize}
...
\textbf{Shrinkage formula:}
\[
\hat{\Sigma} = \lambda\, T + (1-\lambda)\, S
\]
where $S$ = sample covariance, $T$ = stable target matrix, $\lambda\in[0,1]$ = shrinkage intensity.

\textbf{Parameter meaning:}
\begin{itemize}\setlength{\itemsep}{2pt}
  \item $\lambda \to 1$: highly stable, biased toward target $T$.
  \item $\lambda \to 0$: data-driven, less shrinkage, noisier.
\end{itemize}
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{low shrinkage ($\lambda=0.2$)}

\begin{center}
\includegraphics[width=0.8\linewidth]{figures/fig_0053.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0054.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0055.png}\\
\includegraphics[width=0.8\linewidth]{figures/fig_0056.png}
\end{center}


\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $S=0.10$, $T=0.06$, $\lambda=0.2$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\hat{\Sigma} = 0.2\times0.06 + 0.8\times0.10 = 0.012 + 0.080 = 0.092$
\end{itemize}

\noindent\textbf{Interpretation:} Low $\lambda$ keeps the estimate close to the sample matrix $S$.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{high shrinkage ($\lambda=0.8$)}

\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $S=0.10$, $T=0.06$, $\lambda=0.8$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\hat{\Sigma} = 0.8\times0.06 + 0.2\times0.10 = 0.048 + 0.020 = 0.068$
\end{itemize}

\noindent\textbf{Interpretation:} High $\lambda$ pulls the estimate strongly toward the stable target $T$.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{intermediate shrinkage ($\lambda=0.3$)}

\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $S=0.12$, $T=0.05$, $\lambda=0.3$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\hat{\Sigma} = 0.3\times0.05 + 0.7\times0.12 = 0.015 + 0.084 = 0.099$
\end{itemize}

\noindent\textbf{Interpretation:} Result lies between $T$ and $S$; nearer to $S$ because $\lambda$ is small.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{sensitivity to $\lambda$}

\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $S=0.15$, $T=0.08$; compare $\lambda=0.2$ vs $\lambda=0.7$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\lambda=0.2$: $\hat{\Sigma} = 0.2\times0.08 + 0.8\times0.15 = 0.016+0.120 = 0.136$
  \item $\lambda=0.7$: $\hat{\Sigma} = 0.7\times0.08 + 0.3\times0.15 = 0.056+0.045 = 0.101$
\end{itemize}

\noindent\textbf{Interpretation:} Higher $\lambda$ moves the estimate away from the noisy sample matrix.
...
\[
\text{Cov}_t = \lambda\,\text{Cov}_{t-1} + (1-\lambda)\,r_{A,t-1}\,r_{B,t-1}
\]
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{variance update ($\lambda=0.94$)}

\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\sigma_{t-1}^2=0.04$, $r_{t-1}=10\%$, $\lambda=0.94$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\sigma_t^2 = 0.94\times0.04 + 0.06\times(0.10)^2$
  \item $= 0.0376 + 0.0006 = 0.0382$
\end{itemize}

\noindent\textbf{Interpretation:} The new variance is only slightly lower because most weight stays on the prior.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{variance update ($\lambda=0.80$)}

\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Same inputs as Example~1; $\lambda=0.80$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\sigma_t^2 = 0.80\times0.04 + 0.20\times(0.10)^2$
  \item $= 0.0320 + 0.0020 = 0.034$
\end{itemize}

\noindent\textbf{Interpretation:} Lower $\lambda$ reacts more aggressively to the recent return.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{two-period update ($\lambda=0.90$)}

\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\sigma_0^2=0.05$, $r_1=8\%$, $r_2=12\%$, $\lambda=0.90$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Period 1: $\sigma_1^2 = 0.90\times0.05 + 0.10\times(0.08)^2 = 0.0450+0.00064 = 0.04564$
  \item Period 2: $\sigma_2^2 = 0.90\times0.04564 + 0.10\times(0.12)^2 \approx 0.04108+0.00144 = 0.0425$
\end{itemize}

\noindent\textbf{Interpretation:} Each period, the estimate blends the prior variance with the new squared return.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{covariance update ($\lambda=0.95$)}

\noindent\textbf{Given:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\text{Cov}_{t-1}=0.02$, $r_A=10\%$, $r_B=8\%$, $\lambda=0.95$
\end{itemize}
\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\text{Cov}_t = 0.95\times0.02 + 0.05\times(0.10)(0.08)$
  \item $= 0.019 + 0.0004 = 0.0194$
\end{itemize}

\noindent\textbf{Interpretation:} EWMA covariance tracks co-movement with more weight on recent joint returns.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 5}\enspace\textit{2-asset portfolio EWMA risk update}

\noindent\textbf{Given:} $\sigma^2_{A,t-1}=0.04$, $\sigma^2_{B,t-1}=0.09$, $\text{Cov}_{AB,t-1}=0.015$; $r_A=10\%$, $r_B=8\%$; $\lambda=0.94$; $w_A=0.60$, $w_B=0.40$.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\sigma^2_{A,t} = 0.94\times0.04 + 0.06\times(0.10)^2 = 0.0376+0.0006 = 0.0382$
  \item $\sigma^2_{B,t} = 0.94\times0.09 + 0.06\times(0.08)^2 = 0.0846+0.000384 = 0.08498$
  \item $\text{Cov}_{AB,t} = 0.94\times0.015 + 0.06\times(0.10)(0.08) = 0.01410+0.00048 = 0.01458$
  \item $\sigma^2_p = (0.6)^2(0.0382)+(0.4)^2(0.08498)+2(0.6)(0.4)(0.01458)$
  \item $= 0.36\times0.0382 + 0.16\times0.08498 + 0.48\times0.01458 = 0.01375+0.01360+0.00700 = 0.03435$
\end{itemize}

\noindent\textbf{Interpretation:} The two-asset portfolio variance updates to 0.03435 ($\sigma_p\approx18.5\%$) reflecting the blended EWMA inputs. Source figure of 0.03185 uses slightly different prior values but the method is identical.
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{3-window rolling covariance (3-day window)}

\noindent\textbf{Given:} 5-day returns: $r_A$: 2\%, 3\%, 4\%, 6\%, 8\%; $r_B$: 1\%, 3\%, 5\%, 6\%, 9\%.

\noindent\textbf{Window 1 (days 1--3):} $\bar{r}_A=3\%$, $\bar{r}_B=3\%$
\[
\text{Cov}_1 = \frac{(2-3)(1-3)+(3-3)(3-3)+(4-3)(5-3)}{2}\times10^{-4}
= \frac{2+0+2}{2}\times10^{-4} = 0.0002
\]

\noindent\textbf{Window 2 (days 2--4):} $\bar{r}_A=4.33\%$, $\bar{r}_B=4.67\%$
\[
\text{Cov}_2 = \frac{(3-4.33)(3-4.67)+(4-4.33)(5-4.67)+(6-4.33)(6-4.67)}{2}\times10^{-4} \approx 0.00022
\]

\noindent\textbf{Window 3 (days 3--5):} $\bar{r}_A=6\%$, $\bar{r}_B=6.67\%$
\[
\text{Cov}_3 = \frac{(4-6)(5-6.67)+(6-6)(6-6.67)+(8-6)(9-6.67)}{2}\times10^{-4}
= \frac{3.33+0+4.67}{2}\times10^{-4} = 0.0004
\]

\noindent\textbf{Interpretation:} Covariance rises across windows ($0.0002 \to 0.00022 \to 0.0004$) --- the two assets are becoming more positively co-moved over time.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{deteriorating diversification benefit}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Window 1 covariance = 0.0002 (low); portfolio $\sigma_p$ benefits from diversification.
  \item Window 3 covariance = 0.0004 (doubled) $\Rightarrow$ portfolio variance rises even if individual $\sigma$s unchanged.
  \item Formula: $\sigma_p^2 = w_A^2\sigma_A^2 + w_B^2\sigma_B^2 + 2w_Aw_B\text{Cov}_{AB}$. Higher Cov increases $\sigma_p^2$ directly.
\end{itemize}

\noindent\textbf{Interpretation:} Rising rolling covariance signals that diversification benefit is deteriorating --- assets are moving together more closely over time.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{covariance trend and portfolio impact}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Covariance sequence: $0.000178 \to 0.0005 \to 0.0006$ (from source).
  \item Each jump indicates strengthening positive co-movement.
  \item With $w_A=w_B=0.5$: $\Delta\sigma_p^2 = 2(0.5)(0.5)(0.0006-0.000178) = 0.000422$ extra variance added.
\end{itemize}

\noindent\textbf{Interpretation:} A tripling of covariance adds measurable portfolio risk. An investor relying on a fixed historical covariance estimate would systematically underestimate risk.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{exam-ready interpretation rules}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Increasing rolling covariance $\Rightarrow$ higher portfolio risk concentration; diversification weakening.
  \item Decreasing rolling covariance $\Rightarrow$ diversification benefit strengthening; portfolio risk falls.
  \item Covariance near zero $\Rightarrow$ maximum diversification benefit; assets near-uncorrelated.
  \item Negative covariance $\Rightarrow$ one asset acts as a hedge for the other.
\end{itemize}

\noindent\textbf{Interpretation:} Rolling covariance tracking is essential for dynamic portfolio management. Static covariance estimates miss regime changes and can lead to underestimated risk.
...
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 1}\enspace\textit{basic allocation}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item V=10,00,000; F=8,00,000; m=3
  \item C=2,00,000
  \item E=6,00,000; B=4,00,000
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{after +10\% move}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item New V=10,60,000
  \item C=2,60,000
  \item E=7,80,000; B=2,80,000
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{after -20\% move}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item New V=8,80,000
  \item C=80,000
  \item E=2,40,000; B=6,40,000
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{drawdown floor}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Peak=12,00,000; max drawdown=25\%
  \item Floor=9,00,000
  \item For current V=11,00,000 and m=4: C=2,00,000 $\Rightarrow$ E=8,00,000; B=3,00,000
\end{itemize}

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 5}\enspace\textit{multi-period CPPI (3 periods, no floor breach)}

\noindent\textbf{Given:} $V_0=$ INR~10{,}00{,}000; $F=$ INR~8{,}00{,}000; $m=3$.\\
Returns: Period 1 $+10\%$, Period 2 $-5\%$, Period 3 $+8\%$.

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item \textbf{Period 0:} $C=2{,}00{,}000$; $E=6{,}00{,}000$; $B=4{,}00{,}000$
  \item \textbf{Period 1} ($+10\%$ on risky): New risky $=6{,}00{,}000\times1.10=6{,}60{,}000$; $V_1=6{,}60{,}000+4{,}00{,}000=10{,}60{,}000$\\
        Rebalance: $C=2{,}60{,}000$; $E=7{,}80{,}000$; $B=2{,}80{,}000$ \checkmark
  \item \textbf{Period 2} ($-5\%$ on risky): New risky $=7{,}80{,}000\times0.95=7{,}41{,}000$; $V_2=7{,}41{,}000+2{,}80{,}000=10{,}21{,}000$\\
        Rebalance: $C=2{,}21{,}000$; $E=6{,}63{,}000$; $B=3{,}58{,}000$ \checkmark
  \item \textbf{Period 3} ($+8\%$ on risky): New risky $=6{,}63{,}000\times1.08=7{,}16{,}040$; $V_3=7{,}16{,}040+3{,}58{,}000=10{,}74{,}040$\\
        $C=2{,}74{,}040$; $E=8{,}22{,}120$; $B=2{,}51{,}920$ \checkmark
\end{itemize}

\noindent\textbf{Interpretation:} Floor of INR~8{,}00{,}000 maintained throughout all 3 periods. In the up-down-up market, CPPI auto-reduces risky exposure in Period~2 (protecting the floor) and re-increases in Period~3 (capturing the recovery).

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 6}\enspace\textit{gap risk: aggressive multiplier + crash scenario}

\noindent\textbf{Given:} $V_0=$ INR~10{,}00{,}000; $F=$ INR~9{,}00{,}000 (tight floor); $m=5$ (aggressive).

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $C=10{,}00{,}000-9{,}00{,}000=1{,}00{,}000$; $E=5\times1{,}00{,}000=5{,}00{,}000$; $B=5{,}00{,}000$
  \item \textbf{Overnight crash} $-30\%$ (gap --- no intraday rebalancing possible):
  \item Risky $=5{,}00{,}000\times0.70=3{,}50{,}000$; $V_1=3{,}50{,}000+5{,}00{,}000=8{,}50{,}000$
  \item $C_1=8{,}50{,}000-9{,}00{,}000=-50{,}000$ \textbf{(negative cushion --- floor breached!)}
  \item New $E=5\times(-50{,}000)=-2{,}50{,}000$ (impossible: algorithm locks into all safe assets)
\end{itemize}

\noindent\textbf{Interpretation:} Gap risk materializes when the market crashes faster than rebalancing frequency. High $m=5$ amplifies risky exposure making the portfolio vulnerable. Once floor is breached, CPPI locks all remaining funds in safe assets --- the upside is forfeited permanently. This is the key weakness of CPPI.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 7}\enspace\textit{GBM-simulated monthly shock applied to CPPI}

\noindent\textbf{Given:} $V_0=$ INR~10{,}00{,}000; $F=8{,}00{,}000$; $m=3$; Risky asset: $\mu=12\%$ p.a., $\sigma=20\%$ p.a. Random GBM shock $Z=-1.5$ (downward).

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item Initial: $C=2{,}00{,}000$; $E=6{,}00{,}000$; $B=4{,}00{,}000$
  \item Monthly return from GBM: $r = \left(\mu-\tfrac{1}{2}\sigma^2\right)\tfrac{1}{12}+\sigma\sqrt{\tfrac{1}{12}}\cdot Z$
        $= (0.12-0.02)\times\tfrac{1}{12}+0.20\times0.2887\times(-1.5) = 0.00833-0.08660 = -7.83\%$
  \item New risky $=6{,}00{,}000\times(1-0.0783)=6{,}00{,}000\times0.9217=5{,}53{,}020$
  \item $V_1=5{,}53{,}020+4{,}00{,}000=9{,}53{,}020$
  \item Rebalance: $C_1=9{,}53{,}020-8{,}00{,}000=1{,}53{,}020$; $E_1=3\times1{,}53{,}020=4{,}59{,}060$; $B_1=4{,}93{,}960$
\end{itemize}

\noindent\textbf{Interpretation:} A $-1.5\sigma$ shock ($-7.83\%$ monthly) reduces the cushion from 2{,}00{,}000 to 1{,}53{,}020 and auto-reduces risky allocation from 6{,}00{,}000 to 4{,}59{,}060. Floor of 8{,}00{,}000 is safely preserved ($V_1=9{,}53{,}020$). This demonstrates CPPI's automatic de-risking under GBM-simulated adverse shocks.
...
\item
  Formula: $S = \dfrac{R_p - R_f}{\sigma_p}$
\item
...
\item
  Formula: $T = \dfrac{R_p - R_f}{\beta_p}$
\item
...
\noindent\textbf{Common data for Examples 1--5:}
\begin{center}
\begin{tabular}{|l|r|r|r|r|}
\hline
\textbf{Fund} & \textbf{$R_p$ (\%)} & \textbf{$\sigma_p$ (\%)} & \textbf{$\beta_p$} & \textbf{$R_f=6\%$, $R_m=13\%$}\\
\hline
A & 16 & 20 & 1.2 & \\
B & 14 & 15 & 0.9 & \\
C & 18 & 25 & 1.5 & \\
...
\noindent\textbf{Example 1}\enspace\textit{Jensen's Alpha: rank Funds A, B, C}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\alpha = R_p - [R_f + \beta_p(R_m - R_f)]$
  \item $\alpha_A = 16 - [6 + 1.2\times7] = 16 - 14.4 = +1.6\%$
  \item $\alpha_B = 14 - [6 + 0.9\times7] = 14 - 12.3 = +1.7\%$
  \item $\alpha_C = 18 - [6 + 1.5\times7] = 18 - 16.5 = +1.5\%$
  \item Ranking: $B (1.7\%) > A (1.6\%) > C (1.5\%)$
\end{itemize}

\noindent\textbf{Interpretation:} Fund B generates the highest abnormal return per unit of systematic risk. All three outperform CAPM prediction, but B does so most efficiently given its $\beta$.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 2}\enspace\textit{Sharpe Ratio: rank all 5 funds}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $S = (R_p - R_f)/\sigma_p$
  \item $S_A = (16-6)/20 = 0.500$
  \item $S_B = (14-6)/15 = 0.533$
  \item $S_C = (18-6)/25 = 0.480$
  \item $S_D = (10-6)/18 = 0.222$
  \item $S_E = (15-6)/22 = 0.409$
  \item Ranking: $B(0.533) > A(0.500) > C(0.480) > E(0.409) > D(0.222)$
\end{itemize}

\noindent\textbf{Interpretation:} Fund B delivers the most excess return per unit of total risk. Fund D has the worst risk-adjusted performance despite having lower volatility than C.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 3}\enspace\textit{Treynor Ratio: rank all 5 funds}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $T = (R_p - R_f)/\beta_p$
  \item $T_A = (16-6)/1.2 = 8.33$
  \item $T_B = (14-6)/0.9 = 8.89$
  \item $T_C = (18-6)/1.5 = 8.00$
  \item $T_D = (10-6)/0.8 = 5.00$
  \item $T_E = (15-6)/1.1 = 8.18$
  \item Ranking: $B(8.89) > A(8.33) > E(8.18) > C(8.00) > D(5.00)$
\end{itemize}

\noindent\textbf{Interpretation:} Treynor uses only systematic ($\beta$) risk. Fund E ranks higher under Treynor than Sharpe, because its idiosyncratic risk inflates $\sigma_p$ more than it inflates $\beta_p$.

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 4}\enspace\textit{Jensen's Alpha: all 5 funds including negative alpha}

\noindent\textbf{Steps:}
\begin{itemize}\setlength{\itemsep}{3pt}
  \item $\alpha_A = 16-14.4 = +1.6\%$
  \item $\alpha_B = 14-12.3 = +1.7\%$
  \item $\alpha_C = 18-16.5 = +1.5\%$
  \item $\alpha_D = 10-[6+0.8\times7] = 10-11.6 = -1.6\%$ \textbf{(underperforms CAPM!)}
  \item $\alpha_E = 15-[6+1.1\times7] = 15-13.7 = +1.3\%$
  \item Ranking: $B > A > C > E > D$
\end{itemize}

\noindent\textbf{Interpretation:} Fund D delivers less than its CAPM-required return ($\alpha=-1.6\%$) --- it destroys alpha. All others outperform but by varying degrees. Jensen's Alpha directly answers ``Did the manager add value above market risk compensation?''

\noindent\rule{\linewidth}{0.4pt}
\medskip
\noindent\rule{\linewidth}{0.4pt}
\noindent\textbf{Example 5}\enspace\textit{cross-measure ranking comparison and key insight}

\noindent\textbf{Steps (summary table for all 5 funds):}
\begin{center}
\begin{tabular}{|l|r|r|r|}
\hline
\textbf{Fund} & \textbf{Sharpe} & \textbf{Treynor} & \textbf{Jensen $\alpha$ (\%)}\\
\hline
A & 0.500 & 8.33 & +1.6\\
B & 0.533 & 8.89 & +1.7\\
C & 0.480 & 8.00 & +1.5\\
...
E & 0.409 & 8.18 & +1.3\\
\hline
\textbf{Best} & \textbf{B} & \textbf{B} & \textbf{B}\\
\hline
\end{tabular}
\end{center}

\noindent\textbf{Interpretation:} Fund B is the top performer by all three measures. Fund D is the worst performer. Note that E ranks higher under Treynor than Sharpe because it has more idiosyncratic (non-systematic) risk. Use Sharpe for undiversified portfolios, Treynor for well-diversified ones, Jensen for manager skill assessment.
