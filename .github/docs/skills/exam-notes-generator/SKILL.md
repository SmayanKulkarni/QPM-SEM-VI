...
This skill is the primary and only user-facing surface for the QPM notes project.
...
**Example of INSUFFICIENT theory:**
```latex
\begin{itemize}
    \item \textbf{Algorithm X}: An iterative method for solving problem Y.
    \item \textbf{Metric Z}: Measures performance using a weighted average.
\end{itemize}
```

**Example of SUFFICIENT theory:**
```latex
\textbf{Algorithm X} is an iterative numerical method designed to approximate the
solution to problem Y when an exact closed-form solution is either unknown or
computationally intractable. Unlike direct methods that attempt to solve the problem
in a single step, Algorithm X refines an initial guess through successive iterations,
progressively reducing the error until a convergence criterion is met. This makes it
particularly well-suited for large-scale problems where matrix factorization would
require excessive memory or time.

The fundamental operation of Algorithm X can be understood through its update rule.
At each iteration $k$, the algorithm receives the current estimate $x^{(k)}$ and
computes a correction term based on the residual $r^{(k)} = b - A x^{(k)}$. The next
estimate is produced by applying a preconditioner $M^{-1}$ to the residual and adding
the result to the current estimate:
\[
x^{(k+1)} = x^{(k)} + M^{-1} r^{(k)}
\]
where $M$ is a preconditioning matrix chosen to approximate $A$ while remaining
easy to invert. The preconditioner $M$ thus serves as an approximate inverse that
accelerates convergence without requiring the full cost of computing $A^{-1}$.

The convergence behavior depends critically on the spectral properties of the iteration
matrix $G = I - M^{-1}A$. The algorithm converges for any initial guess if and only if
the spectral radius $\rho(G) < 1$, and the asymptotic convergence rate is governed by
$\rho(G)$.

\begin{itemize}
    \item \textbf{Preconditioner selection}: $M$ must balance approximation quality
          (how closely $M^{-1}$ resembles $A^{-1}$) against inversion cost.
    \item \textbf{Convergence dependency}: Because each iterate depends on all previous
          residuals, the error evolution is governed by the entire iteration history
          rather than a single step.
    \item \textbf{Parameter sharing}: The same preconditioner is reused at every
          iteration, which avoids recomputing expensive factorizations at each step.
\end{itemize}
```
...
**Multi-page numerical rule**: If an algorithm trace or worked example begins on slide $N$ and continues on slides $N+1, N+2, \ldots$, you must read **all continuation slides** before writing the corresponding `tcolorbox`. Never reproduce only the first slide of a multi-page numerical.
...
- Choose inputs that:
  ...
  - Cover common exam variations (e.g., if source uses 2×2 matrix, add a 3×3; if source shows the happy path, add one with an unseen/zero-probability case).
...
- `.github/reference-notes/cv-modules-4-6/cv-modules-4-6-complete-study-source.tex` — **Reference example of a complete, production-ready LaTeX study document. This is the quality standard for all generated notes.** Includes full preamble, proper sectioning, nested itemization, formatted equations, and tcolorbox callouts. Refer to this file when generating new study notes.
- `outputs/latex/NLTP-v3-complete-study-source.tex` — **Second reference example demonstrating the full numerical augmentation workflow.** Shows 34 worked examples across 5 modules, covering the complete pipeline from source transcription (blue tcolorbox) through agent-constructed additional examples (green tcolorbox), with sequential global numbering, multi-step algorithm traces, DP tables, gate computations, and PCFG tree probability calculations. Use this as the benchmark for numerical density and format in any subject.
