...
tex_path = '/home/smayan/Desktop/QPM/outputs/cv-modules-4-6-complete-study-source.tex'
with open(tex_path, 'r') as f:
    content = f.read()

# I will rewrite the entire content, borrowing text from the existing one.
# But it's easier to just construct a fresh, perfectly ordered LaTeX string using the known texts.

new_latex = r"""\documentclass[11pt,a4paper]{article}
...
\textbf{Given:}
In an image segmentation task, we have the Ground Truth ($A$) and the Model's Prediction ($B$):
\begin{itemize}
...
\textbf{Interpretation:} IoU penalizes false positives and false negatives more heavily than Dice, which is why IoU (0.57) is lower than Dice (0.72). Both metrics are essential for evaluating segmentation accuracy.
\end{tcolorbox}
...
\textbf{Interpretation:} The object is 60 meters away from the stereo camera setup. This shows the inverse relationship: a small disparity ($0.1$ mm) corresponds to a large depth ($60$ m).
\end{tcolorbox}
